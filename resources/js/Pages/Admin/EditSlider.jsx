...
const EditSlider = ({slider,status}) => {
     const [sidebarOpen, setSidebarOpen] = useState(false);
 const { data, setData, post, errors, processing } = useForm({
    top_text: slider?.top_text || "",
    title_white: slider?.title_white ||"",
    title_highlight: slider?.title_highlight || "",

   
  });
const[preview, setPreview]=useState(
  slider?.photo? `/storage/${slider.photo}`:null
);

const submit=(e)=>{
  e.preventDefault();
  post(`/admin/slider-update/${slider.id}`,{
    forceFormData:true,
  });
};
