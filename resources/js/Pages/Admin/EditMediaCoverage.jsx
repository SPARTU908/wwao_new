...
const EditMediaCoverage = ({ mediaCoverages }) => {
    
    const [sidebarOpen, setSidebarOpen] = useState(false);

    const { data, setData, post, processing, errors } = useForm({
        title: mediaCoverages?.title || "",
        image: null,
    });

    const [preview, setPreview] = useState(
        mediaCoverages?.image ? `/storage/${mediaCoverages.image}` : null
    );

  const submit = (e) => {
    e.preventDefault();

   post(`/admin/update-media/${mediaCoverages.id}`, {
    forceFormData: true,
    });
};
...
export default EditMediaCoverage;