          ...
          <div className="page-content container container-plus">
            <div className="page-header mb-2 pb-2 flex-column flex-sm-row align-items-start align-items-sm-center py-25 px-1">
              ...
              <div className="page-tools mt-3 mt-sm-0 mb-sm-n1">
                ...
                <a href='/admin/tributes-create' className='btn btn-primary' >Add</a>
              </div>
            </div>
 {flash?.success && (
...
>
  <thead className="sticky-nav text-secondary-m1 text-uppercase text-85">
    <tr>
      ...
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Tribute Name</th>
     
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Date Of Birth</th>
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Date Of Death</th>
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">place</th>
      

       <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">description</th>
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Photo</th>
...
{tributes.map((item) => (
    <tr key={item.id} className="d-style bgc-h-default-l4">
      ...
      <td>
        <span className="text-105">{item.name}</span>
        
      </td>

      <td className="text-grey">{item.dob}</td>
      <td className="text-grey">{item.dod}</td>
      <td className="text-grey">{item.place}</td>
   ...
   <td className="align-middle" style={{ width: '20%' }}>
        <a href={`/admin/tributes-edit/${item.id}`}>
  ...
  onClick={(e) => {
    ...
    if (confirm('Are you sure you want to delete this Tribute?')) {
      router.delete(`/admin/tributes-delete/${item.id}`);
    }
  }}
>
