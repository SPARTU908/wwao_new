...
const ChapterEvent = () => {
   ...
   const { flash,chapter_events } = usePage().props;
          ...
          <div className="page-content container container-plus">
            <div className="page-header mb-2 pb-2 flex-column flex-sm-row align-items-start align-items-sm-center py-25 px-1">
              ...
              <div className="page-tools mt-3 mt-sm-0 mb-sm-n1">
                ...
                <a href='/admin/add-chapter-event/create' className='btn btn-primary' >Add</a>
              </div>
            </div>
 {flash?.success && (
...
>
  <thead className="sticky-nav text-secondary-m1 text-uppercase text-85">
    <tr>
      <th className="td-toggle-details border-0 bgc-white shadow-sm"> S.No.
       ...
       <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Chapter Menu</th>
          <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Event Title </th>
          <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Evenet Image</th>
          <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Event Heading </th>
          <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Evenet Description</th>
  ...
  <tbody>
  {chapter_events.map((item,index) => (
   <tr key={item.id} className="d-style bgc-h-default-l4">
      <td className="td-toggle-details pos-rel">{index+1}</td>
      <td className="td-toggle-details pos-rel">{item.Chapter_Name}</td>
      <td>{item.event_title}</td>

      {/* <td>
        {item.event_image && (
          <img
            src={`/storage/${item.event_image}`}
            alt={item.event_image}
            width="50"
          />
        )}
      </td> */}
      ...
      <td>
  {item.event_image &&
    JSON.parse(item.event_image).map((img, index) => (
      <img
        key={index}
        src={`/storage/${img}`}
        alt="event"
      ...
      />
       ...
       <a href={`/admin/edit-chapter-event/${item.id}`}>
  ...
  onClick={(e) => {
    ...
    if (confirm('Are you sure you want to delete this chapter event?')) {
      router.delete(`/admin/chapter-delete-event/${item.id}`);
    }
  }}
>
