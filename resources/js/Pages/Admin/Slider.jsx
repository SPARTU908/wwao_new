          ...
          <div className="page-content container container-plus">
            <div className="page-header mb-2 pb-2 flex-column flex-sm-row align-items-start align-items-sm-center py-25 px-1">
              ...
              <div className="page-tools mt-3 mt-sm-0 mb-sm-n1">
                ...
                <a href='/admin/slider-create' className='btn btn-primary' >Add</a>
              </div>
            </div>
 {flash?.success && (
...
>
  <thead className="sticky-nav text-secondary-m1 text-uppercase text-85">
    <tr>
      ...
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Top Text</th>
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Title White </th>
      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">Title Highlight</th>
...
{slider.map((item) => (
    <tr key={item.id} className="d-style bgc-h-default-l4">
      ...
      <td>
        <span className="text-105">{item.top_text}</span>
        
      </td>

      <td className="text-grey">{item.title_white}</td>
      <td className="text-grey">{item.title_highlight}</td>
  ...
  onClick={(e) => {
    ...
    if (confirm('Are you sure you want to delete this slider?')) {
      router.delete(`/admin/slider-delete/${item.id}`);
    }
  }}
>
