...
const MvssFamily = () => {
           ...
           <div className="page-content container container-plus">
             <div className="page-header mb-2 pb-2 flex-column flex-sm-row align-items-start align-items-sm-center py-25 px-1">
               ...
               <div className="page-tools mt-3 mt-sm-0 mb-sm-n1">
                 ...
                 <a href='/admin/mvss-family-create' className='btn btn-primary' >Add</a>
               </div>
             </div>
  {flash?.success && (
 ...
 >
  ...
  <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">name</th>
   <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">dob</th>
    <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">wife name</th>    


      <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">address</th>
   <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">mobile</th>
    <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">husband photo</th>  
 <th className="border-0 bgc-white bgc-h-yellow-l3 shadow-sm">wife photo</th>  
  ...
  <td>{item.address}</td>
    <td>{item.mobile}</td>
       <td>
         {item.husband_photo && (
           <img
             src={`/storage/${item.husband_photo}`}
             alt={item.name}
             width="50"
           />
         )}
    ...
    <td className="align-middle" style={{ width: '20%' }}>
         <a href={`/admin/mvss-family-edit/${item.id}`}>
   ...
   onClick={(e) => {
     ...
     if (confirm('Are you sure you want to delete this mvss family?')) {
       router.delete(`/admin/mvss-family-delete/${item.id}`);
     }
   }}
 >
